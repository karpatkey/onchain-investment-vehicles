...
|---|---|
| `KpkOivFactory` | `0xD74d1DD6E679adD095B7044c74280de058081FE9` |
| `KpkSharesDeployer` | `0x8B3f1103d5AE1C500D3E0e220cbD597C3E41EebB` |
| `CcipOivDeployer` (orchestrator) | `0xfFb21de510d4b0ac87823e2936AB42855aF69b03` |
| `KpkTimelockDeployer` | `0x411d94a2f56393546f3457f0e4aEfE98f1053dEd` |
...
**Source-verified on six independent backends**, every figure confirmed by a read rather than a tool exit code: **Sourcify v2** 57/57 (3 contracts × all 19 chains, exact-match, and it auto-propagated to Etherscan + Routescan + Blockscout); **Etherscan V2** 51/51 across its 17 chains (`scroll` and `ink` are off V2 — Blockscout covers them); **Blockscout** 30/30 on 10 instances; **OKLink** 30/30 on its 10 chains (ethereum, optimism, gnosis, base, arbitrum, bnb, polygon, avalanche, linea, scroll); **Routescan** 15/15 on its 5 chains; **Tenderly** 54/57 public (hyperevm is not a supported Tenderly network). `Empty` is covered on Etherscan V2 + Blockscout only — a no-logic contract fails every exact-bytecode-match backend, which is structural rather than a gap. Full matrix in the `verification` block of [`script/deployed-infra.json`](../script/deployed-infra.json).

> **OKLink re-verification.** Use [`script/verify/oklink_verify.py`](../script/verify/oklink_verify.py) — it is idempotent and safe to re-run. Read its responses carefully: a submit answering `{"code":"50026","msg":"System error, please try again later."}` means **already verified**, *not* rate-limiting (that misreading cost a session and left this backend recorded as indeterminate when it was complete). `code 0` means the target was *not* verified and a job was just queued, so the sweep doubles as both the status check and the fix.
