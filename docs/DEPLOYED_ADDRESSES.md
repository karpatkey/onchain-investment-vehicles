...
**Deployed on 19 chains, owned by the Safe, verified on-chain** (owner()/wiring invariants checked post-deploy; note these are **not** block-explorer source-verified — deployed without `--verify`, so `explorerVerified` is `false` in `deployed-infra.json`): ethereum, optimism, gnosis, base, arbitrum, bnb, polygon, avalanche, celo, linea, scroll, sonic, unichain, worldchain, hyperevm, mantle, plasma, ink, berachain. The mainnet orchestrator's selector registry is seeded for 17 destinations — **note `hyperevm` (999) is deployed but NOT yet in the mainnet registry** (adding it requires a Safe tx now that mainnet is Safe-owned), so fan-out can't target it until then. **Excluded (unfunded):** `bob` + `katana`. Per-chain deploy block / tx in [`script/deployed-infra.json`](../script/deployed-infra.json).
