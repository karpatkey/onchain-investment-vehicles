...
contract KpkOivFactoryV4 is Ownable, ReentrancyGuard {
    // ── Role keys ─────────────────────────────────────────────────────────────
    ...
    function _validateOivConfig(OivConfig calldata config) internal view {
        ...
        // DELIBERATE DEVIATION from the frozen `KpkOivFactory` this path was carried over from.
        // There, `admin == address(this)` produces an ADMINLESS fund and nothing catches it: the
        // grant is a no-op on a role the factory already holds, the renounce removes the only
        // holder, and `_deploySharesProxy`'s assertion — which only checks that the FACTORY does
        // not hold admin — passes precisely because nobody is left. The fund is then permanently
        // unadministrable: no upgrades, no asset changes, no fee changes. Refused here.
    ...
    // ── NAV-priced funds ────────────────────────────────────────────────────────
    //
    // The second fund type. `KpkShares` takes its share price from the OPERATOR as a calldata
    // argument; `KpkSharesNav` derives it on-chain from a NAV calculator and is therefore
    // SINGLE-CHAIN ONLY — `getAccountNav` values one account on one chain, so a multi-chain fund
    // would price itself off a fraction of its own book. Nothing stops this factory from being
    // deployed on many chains; what must not happen is one NAV fund holding assets across several.
    //
    // Unlike the `KpkShares` path, the implementation is NOT minted per fund. `KpkSharesDeployer`
    // works by embedding `type(KpkShares).creationCode` (22,912 bytes) in its own runtime;
    // `KpkSharesNav`'s initcode is 24,704 bytes, which already exceeds EIP-170's 24,576-byte
    // runtime limit, so no contract can carry it. The implementation is deployed once, out of band,
    // and set by the owner. Funds share it — safe, because UUPS upgrades are per-proxy, so one
    // fund's upgrade cannot reach another — but NAV funds do not get the isolated upgrade surface
    // `deployOiv` gives, and the owner is trusted to set a genuine implementation.

    /// @notice The `KpkSharesNav` implementation every NAV fund's proxy delegates to.
    address public navImplementation;

    /// @notice Append-only log of NAV funds deployed by this factory, keyed by id.
    mapping(uint256 => NavFundInstance) public navFunds;

    /// @notice Number of NAV funds deployed; also the id the next deployment will take.
    uint256 public navFundCount;

    /// @notice Full configuration for a NAV fund deployment. Mirrors `OivConfig` field for field,
    ///         differing only in the shares parameters, so the two entry points stay symmetric.
    struct NavFundConfig {
        /// @notice Signer configuration for the Manager Safe. Same trust warning as `OivConfig`.
        SafeConfig managerSafe;
        /// @notice Base salt controlling all deployed addresses, hashed with the caller.
        uint256 salt;
        /// @notice Exec Roles Modifier owner AND `DEFAULT_ADMIN_ROLE` holder on the fund.
        address admin;
        /// @notice Fund initialization parameters. `admin` and `safe` are OVERWRITTEN — `safe` with
        ///         the freshly deployed Avatar Safe, `admin` with this factory for the duration of
        ///         the call. Whatever is passed in those two fields is ignored.
        ...
        ///         priceable in the NAV calculator, or the deployment reverts.
        AssetConfig[] additionalAssets;
    }

    /// @notice Addresses of the seven contracts `deployNavFund` produces.
    struct NavFundInstance {
        ...
        address subRolesModifier;
        address managerRolesModifier;
        /// @notice The SHARED implementation, not a per-fund one. See the section note above.
        address navImpl;
        address navProxy;
    }

    /// @notice Emitted when the shared NAV implementation is set or replaced.
    ...
    /// @notice `deployNavFund` was called before an implementation was configured.
    error NavImplementationNotSet();

    /// @notice An address that must hold code does not.
    error NotAContract();

    /// @notice The configured admin is this factory, which would leave the fund with no admin.
    error AdminIsFactory();

    /// @notice The role handover did not land on the intended holders.
    error RoleHandoverFailed();

    /// @notice Sets the shared `KpkSharesNav` implementation used by future NAV deployments.
    ...
    /// @notice Deploys a complete NAV-priced fund: operational stack + `KpkSharesNav` proxy.
    /// @dev    Permissionless, like `deployOiv`, and structurally identical to it: same stack, same
    ///         approval grants, same module enable/disable window. The five operational-stack
    ///         addresses are produced by the same `_deployAndWireStack` the other two entry points
    ///         use, so a NAV fund's Safes are wired exactly like a `KpkShares` fund's — including
    ///         the MultiSend unwrap adapters.
    ///
    ///         Because the factory is still an enabled Avatar Safe module at that point, this path
    ///         DOES grant the Avatar Safe's approvals to the fund, which a factory composing
    ///         `deployStack` from outside structurally cannot do.
    ///
    ///         Reverts if the NAV calculator rejects the base asset or any additional asset:
    ///         `initialize` and `updateAsset` both gate on the NAV registry, so a fund can never be
    ///         deployed holding an asset it cannot price.
    /// @param  config   NAV fund deployment parameters.
    /// @return instance Addresses of the seven deployed contracts.
        ...
        returns (NavFundInstance memory instance)
    {
        ...
        // Reserve the registry id before any external call (CEI), so an attacker-controlled ERC-20
        // callback firing during `updateAsset` or the approve transactions cannot shift indices.
        ...
        address proxy = _deployNavProxy(impl, config, stack.managerSafe, stack.avatarSafe);
    ...
    ///      roles. Mirrors `_deploySharesProxy`, minus the per-fund implementation deploy and the
    ///      `isFeeModuleAsset` flag, which `KpkSharesNav` does not have — a single USD share price
    ///      makes the high-water mark one coherent series across every asset.
        ...
        returns (address proxy)
    {
        ...
        proxy = address(
            new ERC1967Proxy{salt: _deriveNavProxySalt(config.salt, msg.sender)}(
                impl, abi.encodeCall(KpkSharesNav.initialize, (params))
            )
        );
        ...
        // Asserted POSITIVELY, unlike `_deploySharesProxy`'s factory-only check. If `config.admin`
        // were this factory the grant would be a no-op on a role it already holds and the renounce
        // would remove the only holder — leaving a fund nobody can administer, which a
        // factory-only assertion passes precisely because nobody is left. `_validateNavFundConfig`
        // refuses that configuration outright; this is the backstop.
        if (
            !fund.hasRole(DEFAULT_ADMIN_ROLE, config.admin) || !fund.hasRole(OPERATOR, operator)
                || fund.hasRole(DEFAULT_ADMIN_ROLE, address(this)) || fund.hasRole(OPERATOR, address(this))
        ) revert RoleHandoverFailed();
    }

    /// @dev Component index 7, continuing the numbering `_deriveSharesSalts` uses for 5 and 6, so a
    ///      NAV fund and a `KpkShares` fund deployed with the same base salt never contend for an
    ///      address. Their init-code differs anyway, which alone makes the CREATE2 addresses differ;
    ///      a distinct index keeps that true even if the two ever came to share an implementation.
    ...
    /// @dev Validates a `NavFundConfig`. The Manager Safe's owners and threshold are validated by
    ///      `_deployAndWireStack`, and every asset's NAV registration by the fund itself.
    function _validateNavFundConfig(NavFundConfig calldata config) internal view {
        _validateManagerOwners(config.managerSafe);
        if (config.admin == address(0)) revert ZeroAddress();
        if (config.admin == address(this)) revert AdminIsFactory();
        if (config.sharesParams.asset == address(0)) revert ZeroAddress();
        if (config.sharesParams.feeReceiver == address(0)) revert InvalidSharesParams();
        if (config.sharesParams.subscriptionRequestTtl == 0 || config.sharesParams.redemptionRequestTtl == 0) {
            revert InvalidSharesParams();
        }
        ...
        for (uint256 i = 0; i < length; i++) {
            address asset = config.additionalAssets[i].asset;
            if (asset == address(0)) revert ZeroAddress();
            // Re-listing the base asset would overwrite its flags with whatever is passed here,
