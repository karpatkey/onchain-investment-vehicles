...
`deployEverywhere` deploys the fund locally and sends stacks to every wired chain that does *not*
carry shares. It skips the other shares chains deliberately — a stack landing on one would
...
forge script script/CcipDeployEverywhere.s.sol --sig "deploy(address,string,uint256[],uint256)" \
